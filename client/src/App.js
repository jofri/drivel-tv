      ...
      <div className="content">
        <Switch>
          <Route exact path="/">
            ...
            <Homepage
              allBroadcasts={allBroadcastObjects}
          ...
          <Route exact path={broadcastUrl}>
            {' '}
            {/* If user visits broadcast page, check if broadcast exists in db - else, redirect to 404*/}
  ...
  async function getAllBroadcasts() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBroadcastUrl('/404');
      } // Else if no broadcasts, send user to 404
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async function getBroadcast(id) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBroadcastUrl('/404');
      } // Else if broadcast does not exist in DB, send user to 404
    } catch (err) {
      console.log(err);
    }
  }
